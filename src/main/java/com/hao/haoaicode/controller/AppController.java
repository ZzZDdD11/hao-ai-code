...
import com.hao.haoaicode.annotation.AuthCheck;
import com.hao.haoaicode.common.BaseResponse;
import com.hao.haoaicode.common.DeleteRequest;
...
public class AppController {
    ...
    @Cacheable(
            value = "good_app_page",
            key = "T(com.hao.haoaicode.utils.CacheKeyUtils).generateKey(#appQueryRequest)",
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    public BaseResponse<AppVO> getAppVOByIdByAdmin(String id) {
        Long appId = Long.parseLong(id);
        ThrowUtils.throwIf(appId <= 0, ErrorCode.PARAMS_ERROR);
        // 查询数据库
        App app = appService.getById(appId);
